...
def hyperbolicity_sample(G, num_samples=2000):
    curr_time = time.time()
    hyps = []
...
def load_UrbanKG_Graph(tiplets):

    """
        k = {0: [14442, 1378, 1544, 6092, 7636], 1: [10199, 8359, 2943]}
        kk = nx.adjacency_matrix(nx.from_dict_of_lists(k))
        graph = nx.from_scipy_sparse_matrix(kk)

    """
    dict_graph = {}
    with open(tiplets, encoding='UTF-8') as f:
        for line in f.readlines():
            temp = line.split(' ')
            dict_graph[ temp[0]] = temp[2]
    kk = nx.adjacency_matrix(nx.from_dict_of_lists(dict_graph))
    graph = nx.from_scipy_sparse_matrix(kk)
...
if __name__ == '__main__':
    ...
    graph_NYC = load_UrbanKG_Graph("../UrbanKG/NYC/triplets_NYC.txt")
    hyp_NYC = hyperbolicity_sample(graph_NYC)
    print('Hyperbolicity: ', hyp_NYC)
    print('Cycle：', len(nx.cycle_basis(graph_NYC, 0)))

    graph_CHI = load_UrbanKG_Graph("../UrbanKG/CHI/triplets_CHI.txt")
    hyp_CHI = hyperbolicity_sample(graph_CHI)
    print('Hyperbolicity: ', hyp_CHI)
    print('Cycle：', len(nx.cycle_basis(graph_CHI, 0)))
